...
app.controller('LogingController', function ($scope) {
    ...
    if (localStorage.FB_user_id) {
        ...
        document.getElementById('User-img').innerHTML = '<img class="img-responsive img-circle"  src="' + localStorage.imgURL + '" width="100" height="100"/>';
    }
    ...
    window.fbAsyncInit = function () {
        ...
        gapi.load('auth2', function () {
            auth2 = gapi.auth2.init({
                client_id: '159925123117-l4dvqh6nbrcbf3thkqpale17qv402eau.apps.googleusercontent.com',
                cookiepolicy: 'single_host_origin',
            });
            Signin(document.getElementById('GoogleReg'));
        });
        ...
        FB.login(function (response) {
            if (response.status === 'connected') {
                FB.api('/me', { fields: 'email,name,id' }, function (response) {
                    ...
                    window.location.href = 'session.php?para1=' + response.email + '&para2=' + response.name;
                });
            } else if (response.status === 'not_authorized') {
    ...
    function Signin(element) {
        auth2.attachClickHandler(element, {}, function (user) {
            ...
            localStorage.user_name = profile.getName();
            localStorage.Google_user_id = profile.getId();
            localStorage.isAuthenticated = true;
            localStorage.imgURL = profile.getImageUrl();
            document.getElementById('User-img').innerHTML = '<img class="img-responsive img-circle" src="' + localStorage.imgURL + '" width="100" height="100"/>';
            window.location.href = 'index.php?para1=' + profile.getEmail() + '&para2=' + profile.getName();
        }, function (error) {
            console.log(error);
        });
